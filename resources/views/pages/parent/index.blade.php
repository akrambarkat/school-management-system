...
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0"> {{ __('main_tans.parent_list') }}
            </h4>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                <li class="breadcrumb-item"><a href="#" class="default-color">{{ __('main_tans.home') }}</a></li>
                <li class="breadcrumb-item active">{{ __('main_tans.parent_list') }}</li>
            </ol>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                <a href="{{ route('add_parent.index') }}"><button type="button" class="btn btn-success"
                        data-toggle="modal">
                        {{ __('main_tans.add_parent') }}
                    </button></a>
                    ...
                    <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                        ...
                        <thead>
                            <tr class="table-success">
                                ...
                                <th>{{ trans('main_tans.parent_job') }}</th>
                                <th>{{ trans('main_tans.action') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $i = 0; ?>
                            @foreach ($my_parents as $x)
                                <tr>
                                    <?php $i++; ?>
                                    <td>{{ $i }}</td>
                                    <td>{{ $x->trans_name }}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form id="deleteForm" action="{{ route('parent.destroy', $x->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
