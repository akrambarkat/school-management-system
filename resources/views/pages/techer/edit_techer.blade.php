...
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0"> {{ __('main_tans.edit_techer') }}
            </h4>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                ...
                <li class="breadcrumb-item active"> {{ __('main_tans.add_techer') }}
                </li>
            </ol>
        </div>
    </div>
    ...
    <div class="col-md-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                ...
                <h5>{{ __('main_tans.techer_information') }}</h5><br>
                <form action="{{ route('teacher.update', $y->id) }}" method="POST" autocomplete="off">
                    @method('put')
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            <div class="col-md-6">
                                <label for="name_ar">{{ __('main_tans.name_techer_ar') }}</label>
                            ...
                            <div class="col-md-6">
                                <label for="name_en">{{ __('main_tans.name_techer_en') }}</label>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-3">
                                <label for="title">{{ trans('main_tans.Joining_Date') }}</label>

                                <input class="form-control" type="text" id="datepicker-action" name="Joining_Date"
                                    data-date-format="yyyy-mm-dd" required
                                    value="{{ old('Joining_Date', $y->joining_data) }}">
                        ...
                        <div class="form-group row">
                            <div class="col-md-6">
                                <label for="notes_en">{{ __('main_tans.Specialization') }}</label>
                                <select name="specialization_id" id="specialization_id" class="form-control">
                                    ...
                                    @foreach ($specialization as $x)
                                        <option value="{{ $x->id }}"
                                            {{ old('specialization_id', $y->specialization_id) == $x->id ? 'selected' : '' }}>
                                            {{ $x->trans_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="gender_id" id="gender_id" class="form-control">
                                    ...
                                    @foreach ($gender as $x)
                                        <option value="{{ $x->id }}"
                                            {{ old('gender_id', $y->gender_id) == $x->id ? 'selected' : '' }}>
                                            {{ $x->trans_name }}
                                        </option>
                                    @endforeach
                                </select>
